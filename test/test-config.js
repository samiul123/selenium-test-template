...
import {BrowserType, DeviceName, RunnerOptions, VisualGridRunner, Eyes, Configuration, ScreenOrientation, BatchInfo} from "@applitools/eyes-selenium";

let driver = await new Builder()
    .forBrowser('chrome')
    .build();
const runnerOptions = new RunnerOptions().testConcurrency(1);
let runner = new VisualGridRunner(runnerOptions);

// Create Eyes object with the runner, meaning it'll be a Visual Grid eyes.
let eyes = new Eyes(runner);
function exampleBeforeEach() {
    console.log("Before each example suits tests")
...
async function exampleAfterEach() {
    console.log("After each example suits tests")
        // Close the browser.
    await driver.quit();
...
export {exampleBeforeEach, exampleAfterEach}
export {driver, eyes}
